...
class Transformer():
    ...
    def __init__(self, cc_transcription, repl_file, basechrs_file, agids_file, chardecl_file, teiskeleton_file, output_file):
        # was auf jeden Fall machen: Ausgangsdatei einlesen, 
        # check if all documents there (normalize csv und basecharacters csv)
    ...
    def plain_trans(self, vowels_bool=True):             # from CC-xml file to approach 1
        ...
        self._replace_subs()
        #fulltext = ' '.join(self.mod_root.itertext())            # extracts full text as string
    ...
    def _get_basechr(self, letter, pos):
        ...
        if len(row)>1: # if different definitions based on position: check position and get 1 row
            ...
            row = row.iloc[0]   # will die row auf jeden Fall als Series
            
        return row.loc['ag']
    
    def _normalize_structure(self):
        self._iter_normalize_chrs()
        self._replace_subs()                           #hier wird ein self.root und self.tree erstellt
        self._transform_structure()                    #Vokale und base characters zerlegt und in Attribute
        #prüfen
    ...
    def _transform_structure(self):
        ...
        for w in self.mod_root.findall('.//w'): # loop through all <w> elements
            ...
            g = True                                        # g ist True, wenn der Buchstabe ein Graphem ist
